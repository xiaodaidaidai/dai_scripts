...
def process_file(input_file):
    ...
    with open(input_file, 'r') as infile:
        for line in infile:
            ...
            length = abs(int(fields[4]) - int(fields[3])) + 1
            ...
            # 统计 snRNA 和 snoRNA 及其子类型
            elif rna_type in ["snRNA", "snoRNA"]:
                stat.setdefault("snRNA", [0, 0])
                stat["snRNA"][0] += 1
                stat["snRNA"][1] += length

                subtype_match = re.search(r'subtype=(.*)', fields[8])
                if subtype_match:
...
def write_output(stat, output_file):
    """将统计结果输出到指定文件中。"""
    order = ["miRNA", "tRNA", "rRNA", "5s", "5_8s", "8s", "18s", "28s", "snRNA",
             "CD-box", "HACA-box", "splicing", "scaRNA"]
    ...
    with open(output_file, 'w') as outfile:
        ...
        for rna_type in order:
            ...
            class_type = rna_type if rna_type in ["miRNA", "tRNA", "rRNA", "snRNA"] else ""
